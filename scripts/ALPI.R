...
# Written for R 3.1.2 
# https://waves-vagues.dfo-mpo.gc.ca/Library/359380.pdf # paper reference
# https://rda.ucar.edu/datasets/d010001/ # data
############################################################  
...
alpi.results <- data.frame( year,rbind(alpi2011,alpi2012,alpi2013,alpi2014,alpi2015), row.names = NULL) 
write.table(alpi.results, file="alpi2011-2015.csv",row.names=F,sep=",",quote=F) 
# 
#Save coordinates to a text file in the current working directory  
coords.results <- rbind( t(coords2011), t(coords2012), t(coords2013), t(coords2014), t(coords2015)) 
coords.results <- data.frame( t(matrix(unlist(strsplit(dimnames(coords.results)[[1]]," ")),nrow=2)), coords.results) 
rownames(coords.results) <- NULL 
colnames(coords.results) <- c("month","year","long","lat","x","y") 
write.table(coords.results, file="coords20112015.csv",row.names=F,sep=",",quote=F)
