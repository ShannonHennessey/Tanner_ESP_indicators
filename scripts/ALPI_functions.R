# This is a source file named ALPI.r # Updated for R 3.1.2 
# January 12, 2015 
############################################################                
...
# subtracted from the longterm (1950-1997) average, and the 
...
# library(sp) 
# loaded automatically with 'raster" 
############################################################   
...
slp.idw <- function(month,cs=40){   
  ...
  grd <- spsample(data.new,type = "regular", offset = c(0.5,0.5), cellsize = c(cs*1000,cs*1000))   
  gridded(grd) <- TRUE   
...
get.alpa.ctr <- function(month.ras) { 
  ...
  # you can pick the biggest one. library(raster) library(rgdal) 
  #   
...
seasonal.ctr <- function(data.rcl.list) { 
  ...
  rownames(coord.ctr) <- c("lon","lat","x","y") #get the January year and use to create a colname for the new coords 
